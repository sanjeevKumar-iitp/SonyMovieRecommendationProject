...
#     prompt = f"""You are given {len(reviews)} reviews about the movie "{movie_title}". Read them carefully and generate two short summaries:
...
# # Generate summaries using LLaMA 3
...
#     run_pipeline(
#         json_path="/Volumes/Sanjeev HD/M.TECH  IIT-P/Sem - 3 research/SonyResearchMovieRecommendation/pipeline/output/json/grouped_reviews.json",
#         output_file="summaries_part1.json",
#         MAX_MOVIES=100,               # Change this to 100, 1000, etc.
#         START_OFFSET=0,                # Resume from where you left off
#         MAX_REVIEWS_PER_MOVIE=10       # Use up to N reviews per movie
#     )
...
# Generate summaries using Aryanne/Mistral-3B-Instruct-v0.2-init
def summarize_with_mistral(model, tokenizer, prompt, device):
    inputs = tokenizer(prompt, return_tensors="pt").to(device)
    outputs = model.generate(
        **inputs,
        max_new_tokens=300,
...
def run_pipeline(
    ...
    for idx, imdb_id in tqdm(enumerate(imdb_ids), desc="Processing Movies", total=total_movies):
        ...
        prompt = build_prompt(movie_title, selected_reviews)
        summaries = summarize_with_mistral(model, tokenizer, prompt, device)
